...
public class OrderService : IOrderService
{
    ...
    public async Task<Order> CreateOrderAsync(CreateOrderRequest request)
    {
        ...
        foreach (var it in request.Items)
        {
            ...
            // check inventory before creating the order line
            var inv = await _inventoryRepository.GetByProductIdAsync(it.ProductId);
            if (inv != null && inv.Quantity < it.Quantity)
            {
                throw new InvalidOperationException($"Insufficient stock for product {it.ProductId}. Requested {it.Quantity}, available {inv.Quantity}.");
            }
            ...
            // reduce inventory if exists (we already checked availability above)
            if (inv != null)
            {
